...
Ordinary company export and PDF include native assets, IPAM, readable topology, reconstruction articles, safe completeness gaps, and local source-state dates. The dossier uses local labels and contains the essential procedure text; it is not a collection of Breeze links. Raw upstream organization/source IDs, revisions, fingerprints, provider configuration, rejected values, and secret-derived content are removed. Passwords remain controlled by the export's existing explicit `includePasswords` option; Breeze never changes that setting.
