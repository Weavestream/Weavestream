  ...
  async function submit() {
    ...
    for (const f of activeFields) {
      ...
      if (v === '' || (Array.isArray(v) && v.length === 0)) {
        ...
        payloadValues[f.slug] = serializeTagChips(v as TagChipDraft[]);
      } else {
...
function FieldInput({
  ...
  switch (field.fieldType) {
    ...
    case 'TAGS': {
      ...
      // once, at submit time, in `submit()` via `serializeTagChips`.
      // Pre-migration legacy strings are coerced into best-effort chips
      // here so they still render until the user re-saves.
      const tags: TagChipDraft[] = Array.isArray(value)
        ? (value as unknown[]).flatMap<TagChipDraft>((v) => {
            if (typeof v === 'string') {
        ...
        : [];
      return (
        <TagsInput value={tags} onChange={onChange} disabled={disabled} />
      );
    }
    case 'ASSET_REFERENCE': {
 ...
 * chips carry only a `name`. The form serialises this back to the mixed
 * wire shape `(string | {name})[]` on save — the API resolves `{name}` to
 * a UUID inside the asset-write transaction via `TagsStrategy.preResolve`.
 */
type TagChipDraft = { id?: string; name: string };

type TagSuggestion = { id: string; name: string };

function serializeTagChips(chips: TagChipDraft[]): Array<string | { name: string }> {
