...
- **Color tokens are inherited; geometry tokens are not.** Consume the existing semantic vars from `packages/shared/styles/color-tokens.css` (`--accent`, `--accent-ink`, `--accent-soft`, `--bg`, `--surface`, `--line`, `--text`, `--muted`, `--danger`) rather than literal hexes, so the user's accent preference and any future palette tweak carry over. The deliberate exception is mobile's higher-contrast `--muted` override in `apps/mobile/src/styles/tokens.css`; keep its dark, light, and system-light blocks aligned. Radii, type scale, and touch sizing are mobile's own (9–30px vs desktop's 3–8px). Where the design handoff's hexes differ slightly from the tokens, the tokens win.
